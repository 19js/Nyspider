...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.comboBox_4.addItem("")
        self.comboBox_4.addItem("")
        self.year_combox=QtWidgets.QComboBox(self.centralWidget)
        self.year_combox.setGeometry(QtCore.QRect(660, 60, 70, 30))
        self.month_combox=QtWidgets.QComboBox(self.centralWidget)
        self.month_combox.setGeometry(QtCore.QRect(732, 60, 50, 30))
        self.day_combox=QtWidgets.QComboBox(self.centralWidget)
        self.day_combox.setGeometry(QtCore.QRect(782, 60, 50, 30))
...
class Aso100(QtWidgets.QMainWindow,Ui_MainWindow):
    ...
    def basicInit(self):
        ...
        for i in range(10):
            self.year_combox.addItem(str(int(year)-i))
        for i in range(12):
            month=i+1
            if month<10:
                self.month_combox.addItem('0'+str(month))
            else:
                self.month_combox.addItem(str(month))
        self.month_change()
        self.pushButton.clicked.connect(self.getData)
        self.month_combox.currentIndexChanged.connect(self.month_change)
        self.year_combox.currentIndexChanged.connect(self.month_change)
        month=time.strftime("%m",time.localtime())
        self.month_combox.setCurrentIndex(int(month)-1)
        day=time.strftime("%d",time.localtime())
        self.day_combox.setCurrentIndex(int(day)-1)
        self.action.triggered.connect(self.close)
        for key in self.genre:
            self.comboBox_3.addItem(key)

    def month_change(self):
        year=self.year_combox.currentText()
        month=self.month_combox.currentText()
        self.day_combox.clear()
        days=calendar.monthrange(int(year),int(month))
        for i in range(days[1]):
            day=i+1
            if day<10:
                self.day_combox.addItem('0'+str(day))
            else:
    ...
    def getData(self):
        ...
        year=self.year_combox.currentText()
        month=self.month_combox.currentText()
        day=self.day_combox.currentText()
